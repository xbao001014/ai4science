...
def test_publisher_for_doi_routing():
    ...
    # Elsevier intentionally unsupported (anti-bot cookies)
    assert pd.publisher_for_doi("10.1016/j.compmedimag.2022.102176") is None
...
def test_try_publisher_direct_skips_elsevier_prefix(tmp_path: Path):
...
def test_download_pdf_elsevier_skips_publisher_goes_scansci(monkeypatch, tmp_path: Path):
    """10.1016/ must not call publisher_direct; ScanSci only."""
    import config
    import fetcher.publisher_direct as pubmod
    from fetcher import scansci_fetcher as sf
    ...
    _install_fake_scansci(fake_download)
    result = sf.download_pdf("10.1016/j.ygyno.2021.07.015", "34275655")
    assert called["n"] == 0
    assert result["success"] is True
    assert result["source"] == "unpaywall"
