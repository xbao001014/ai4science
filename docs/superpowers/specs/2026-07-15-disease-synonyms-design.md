...
**Extension:** ZH–EN focus + offline UMLS CUI — see [`2026-07-15-zh-en-focus-synonyms-design.md`](2026-07-15-zh-en-focus-synonyms-design.md) (merged implementation; adds `umls_cui`, `colorectal_polyp` / 肠息肉).
...
| `umls_cui` | Optional offline UMLS CUI (no runtime API) — see ZH–EN extension |
...
Align with existing mapper coverage and add NPC + colorectal polyp:

- `gastric_adenocarcinoma` (胃 / 胃癌 / GC-ADC …)
- `lung_adenocarcinoma` / NSCLC-oriented aliases as today
- `colorectal_adenocarcinoma`
- `colorectal_polyp`（肠息肉 / colorectal polyp — see ZH–EN extension for phrases & FP controls）
...
- MeSH ID join tables.
- Runtime UMLS API; vector focus fallback (see ZH–EN extension).
